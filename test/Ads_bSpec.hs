...
prop_Crc24XorOutUplink xorData xs =
    crc24XorOut xorData' (low3bytes crc' ++ xs) == CrcIsOk where
...
testMsg2 = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xA0]
testMsg3 = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x28]
testMsg4 = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xA8]

testMsg1', testMsg2', testMsg3', testMsg4' :: [Word8]
testMsg1' = [0x5F, 0x66, 0x80, 0x00, 0x00, 0x00, 0x20]
testMsg2' = [0x94, 0x82, 0xC8, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xA0]
testMsg3' = [0xCE, 0x78, 0x20, 0x00, 0x00, 0x00, 0x28]
testMsg4' = [0x4F, 0x15, 0x0B, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xA8]
...
spec = do
    describe "ADS-B" $ do
        ...
        it "short data error test 1" $ do
            evaluate (crc24 [0]) `shouldThrow`
                errorCall errorMessagePrepareData
        it "short data error test 2" $ do
            evaluate (crc24 [0, 1]) `shouldThrow`
                errorCall errorMessagePrepareData
